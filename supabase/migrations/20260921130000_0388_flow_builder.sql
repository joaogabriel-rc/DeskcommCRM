-- 0388: Flow Builder — automações visuais de WhatsApp (estilo ManyChat)
--
...
as $$
  select p_event_type = any (array[
    ...
    -- ⚠️ `contact.created` e `contact.updated` SAÍRAM daqui (migration 0388):
...
-- ═══ A FRONTEIRA DE SERVIÇO RECONHECE OS GATILHOS NOVOS (migration 0388) ═══
--
...
begin
  ...
  -- A 0279 reservou também os eventos de CASO ao servidor: o caso só nasce do
  -- motor. Esta redefinição (0388) é posterior a ela no baseline — e no baseline
  -- vale a ÚLTIMA definição —, então a lista tem de carregar a reserva inteira.
