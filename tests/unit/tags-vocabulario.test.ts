...
describe("fatia S4 — vocabulário de tags (contrato do que foi escrito)", () => {
  ...
  it("o invólucro transacional chama MESMO a operação de vocabulário", () => {
    ...
    expect(sql).toContain("public.fn_vocabulario_de_tags_operar(p_org, p_acao, p_tag, p_destino, p_cor)");
    expect(sql).toContain("public.fn_tag_registro_aplicar(p_org, p_acao, p_tag, p_destino)");
    // A assinatura de 4 argumentos sai por `drop`, pelo motivo que a 0336 mediu:
    // `create or replace` com um parâmetro a mais cria SOBRECARGA, e o PostgREST
    // resolveria a chamada na antiga.
    expect(sql).toMatch(/drop function if exists public\.fn_tag_operar\(uuid, text, text, text\);/);
    // O registro não guarda cor: `definir_cor` é "nada a fazer" ali, e não
    // `acao_invalida` — que desfaria a transação e a cor junto.
    expect(sql).toMatch(/if p_acao = 'definir_cor' then\s+return jsonb_build_object\('registro', 'inalterado'\);/);
    // Fonte ÚNICA da cor e da descrição: `organizations.settings.tags[]`. Uma
    // coluna no registro seria a segunda, sem nada que as mantivesse iguais — e
    // o chip do Inbox (`lib/tags/cor-da-etiqueta.ts`) continuaria lendo a outra.
    const tabela = sql.match(/create table if not exists public\.tags \(([\s\S]*?)\n\);/)?.[1] ?? "";
    expect(tabela).toContain("folder text not null");
    expect(tabela).not.toMatch(/^\s*(color|description)\s+text/m);
