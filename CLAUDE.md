...
- Service role bypassa RLS — handlers que usam admin client **DEVEM** filtrar `organization_id` manualmente, resolvido de fonte confiável (cookie/JWT/webhook secret/path token), **NUNCA do body**
  - **Exceção única e declarada — o webhook universal da Meta** (`/api/v1/webhooks/meta`): o tenant sai do `value.metadata.phone_number_id` de um corpo cuja assinatura HMAC-SHA256 conferiu com o App Secret da instalação — ou, para `message_template_status_update`, que não traz número, da WABA (`entry.id`). Quem é dono do número vem do banco (índice único 0165, só sessão ativa), e dono ausente ou ambíguo não recebe nada; a WABA devolve TODAS as sessões ativas nela (o template é da conta, e cada organização que a conectou o espelha), e cada escrita leva o `organization_id` da sessão. **Custo aceito pelo dono do produto em 22/09/2026:** com o token no caminho (`/meta/[token]`, que segue vivo) forjar um evento exigia o App Secret E o token daquela organização; no universal, o App Secret basta para qualquer organização da instalação. A exceção é por função, não por arquivo, e é presa por `tests/unit/canal-consulta-por-organizacao.test.ts` (`metaSessionByPhoneNumberId` e `metaSessionsByWabaId`: recusa de ambiguidade no número, um único chamador, assinatura antes da busca, escrita escopada pela sessão) — nenhuma outra rota herda essa porta
